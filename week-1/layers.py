...
class LaplacianConvolutionalLayer(torch.nn.Module):
    def __init__(self, weights_array: np.ndarray):
        ...
        self.laplacian_kernel = torch.tensor(weights_array, dtype=torch.float32).reshape(1, 1, 3, 3)
        ##### END OF YOUR CODE #####
    def forward(self, x):
        #### YOUR CODE HERE
        ...
        r, g, b = torch.split(x, split_size_or_sections=[1, 1, 1], dim=1)
        red_out = F.conv2d(r, self.laplacian_kernel, padding='same', groups=1)
        green_out = F.conv2d(g, self.laplacian_kernel, padding='same', groups=1)
        blue_out = F.conv2d(b, self.laplacian_kernel, padding='same', groups=1)
